...
static void ADS7843_SPI_Init(void)
{
    ...
    LPC_PINCON->PINSEL0 &=
        ~((3UL << 14) | (3UL << 16) | (3UL << 18)); /* P0.7,P0.8,P0.9
                                                       cleared
                                                     */
    LPC_PINCON->PINSEL0 |= (2UL << 14) | (2UL << 16) |
                           (2UL << 18); /* P0.7
                                           SCK1,P0.8
                                           MISO1,P0.9
                                           MOSI1 */
...
void DeleteCross(uint16_t Xpos, uint16_t Ypos)
{
    ...
    LCD_DrawLine(
        Xpos - 15, Ypos + 15, Xpos - 7, Ypos + 15, RGB565CONVERT(0, 0, 0));
    LCD_DrawLine(
        Xpos - 15, Ypos + 7, Xpos - 15, Ypos + 15, RGB565CONVERT(0, 0, 0));
    ...
    LCD_DrawLine(
        Xpos + 7, Ypos + 15, Xpos + 15, Ypos + 15, RGB565CONVERT(0, 0, 0));
    LCD_DrawLine(
        Xpos + 15, Ypos + 7, Xpos + 15, Ypos + 15, RGB565CONVERT(0, 0, 0));
    ...
    LCD_DrawLine(
        Xpos + 7, Ypos - 15, Xpos + 15, Ypos - 15, RGB565CONVERT(0, 0, 0));
    LCD_DrawLine(
        Xpos + 15, Ypos - 15, Xpos + 15, Ypos - 7, RGB565CONVERT(0, 0, 0));
}
                             ...
                             Matrix *matrixPtr)
{
    ...
    else
    {
        ...
        matrixPtr->Cn = (screenPtr[2].x * displayPtr[1].x -
                         screenPtr[1].x * displayPtr[2].x) *
                            screenPtr[0].y +
                        (screenPtr[0].x * displayPtr[2].x -
                         screenPtr[2].x * displayPtr[0].x) *
                            screenPtr[1].y +
                        (screenPtr[1].x * displayPtr[0].x -
                         screenPtr[0].x * displayPtr[1].x) *
                            screenPtr[2].y;
        ...
        matrixPtr->Fn = (screenPtr[2].x * displayPtr[1].y -
                         screenPtr[1].x * displayPtr[2].y) *
                            screenPtr[0].y +
                        (screenPtr[0].x * displayPtr[2].y -
                         screenPtr[2].x * displayPtr[0].y) *
                            screenPtr[1].y +
                        (screenPtr[1].x * displayPtr[0].y -
                         screenPtr[0].x * displayPtr[1].y) *
                            screenPtr[2].y;
    }
    return (retTHRESHOLD);
}
...
void TouchPanel_Calibrate(void)
{
    ...
    setCalibrationMatrix(
        &DisplaySample[0], &ScreenSample[0], &matrix); /* ����ֵ�õ�����
                                                        */
}
