...
// unsigned int  CAN_TxRdy[2] = {0,0};              /* CAN HW ready to transmit
// a message */ unsigned int  CAN_RxRdy[2] = {0,0};              /* CAN HW
// received a message */
    ...
    {                           /* Add mask for standard identifiers */
        ...
        { /* For entering second ID */
            if ((LPC_CANAF_RAM->mask[0] >> 16) > id)
                LPC_CANAF_RAM->mask[0] = (LPC_CANAF_RAM->mask[0] >> 16) |
                                         (id << 16);
            else
                LPC_CANAF_RAM->mask[0] = (LPC_CANAF_RAM->mask[0] & 0xFFFF0000) |
                                         id;
        }
        else
        {
            ...
            {                                    /* Adding ID as last entry */
                if ((CAN_std_cnt & 0x0001) == 0) /* Even number of IDs exists */
                    ...
                    LPC_CANAF_RAM->mask[cnt1] =
                        (LPC_CANAF_RAM->mask[cnt1] & 0xFFFF0000) | id;
            }
            else
            {
                ...
                if ((CAN_std_cnt & 0x0001) == 0) /* Even number of IDs exists */
                    LPC_CANAF_RAM->mask[cnt1] =
                        (LPC_CANAF_RAM->mask[cnt1] & 0xFFFF0000) | (0x0000FFFF);
            }
        }
        CAN_std_cnt++;
    }
...
void CAN_Init(void)
{
    ...
    CAN_wrFilter(1, 1, STANDARD_FORMAT); /* Enable reception on CAN 1 of
                                            messages from CAN 2 - TBV */
    CAN_wrFilter(2, 2, STANDARD_FORMAT); /* Enable reception on CAN 2 of
                                            messages from CAN 1 - TBV */
